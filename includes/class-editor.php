...
class Editor_Options {
	...
	public static function register_settings() {
		...
		$heading_default = __( 'Default editor for all users', 'acf' );
		$heading_allow   = __( 'Allow users to switch editors', 'acf' );
	...
	public static function editor_settings_default() {
		acf_include( 'views/partials/settings-writing-editor-default.php' );
	}
	...
	public static function editor_settings_allow() {
		acf_include( 'views/partials/settings-writing-editor-allow.php' );
	}
	...
	public static function user_settings() {
		...
		acf_include( 'views/partials/settings-user-editor-default.php' );
	}
	...
	public static function network_settings() {
		acf_include( 'views/partials/settings-network-editor-default.php' );
	}
	...
	public static function do_meta_box( $post ) {
		...
		<p style="margin: 1em 0;">
			<a href="<?php echo esc_url( $edit_url ); ?>"><?php _e( 'Switch to block editor ', 'acf' ); ?></a>
		</p>
		<?php
	}
	...
	public static function enqueue_block_editor_scripts() {
		...
		wp_enqueue_script(
			'editor-options',
			acf_get_url( 'assets/js/editor-options' . $suffix . '.js' ),
		...
		wp_localize_script(
			'editor-options',
			'editorOptionsL10n',
			[ 'linkText' => __( 'Switch to rich text editor ', 'acf' ) ]
		);
	}
	...
	public static function add_edit_links( $actions, $post ) {
		...
		$text       = _x( 'Edit Blocks', 'Editor Name', 'acf' );
		$label      = sprintf( __( 'Edit &#8220;%s&#8221; in the block editor', 'acf' ), $title );
		...
		$text         = _x( 'Edit Rich Text', 'Editor Name', 'acf' );
		$label        = sprintf( __( 'Edit &#8220;%s&#8221; in the rich text editor', 'acf' ), $title );
	...
	public static function add_post_state( $post_states, $post ) {
		...
		if ( ! $editors['editor_options'] && ! $editors['block_editor'] ) {
			...
			$state = '<span class="editor-options-forced-state">' . _x( 'rich text editor', 'Editor Name', 'acf' ) . '</span>';

		} elseif ( ! $editors['editor_options'] && $editors['block_editor'] ) {

			// Forced to block editor.
			$state = '<span class="editor-options-forced-state">' . _x( 'block editor', 'Editor Name', 'acf' ) . '</span>';
			...
			$state = $is_tinymce ? _x( 'Rich Text', 'Editor Name', 'acf' ) : _x( 'Blocks', 'Editor Name', 'acf' );
		}
...
if ( ! function_exists( 'classicpress_version' ) ) {
	if ( ! is_plugin_active( 'classic-editor/classic-editor.php' ) ) {
		add_action( 'plugins_loaded', [ 'Editor_Options', 'init_actions' ], 11 );
	}
}
