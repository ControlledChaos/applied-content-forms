...
class ACF_Post_Types extends ACF_Module {
	...
	public function menu_icon_preview() {
		...
		$css  = sprintf(
			...
			'.icon-preview { margin: 0; }',
			'.icon-preview:not( .active ) { display: none; }',
			'.icon-preview .dashicons { display: block; box-sizing: border-box; width: 100%; height: unset; text-align: unset; font-size: 3em; }',
			'.icon-preview .dashicons:before { display: block; }'
		);
		...
		foreach ( $icons as $slug => $name ) {
			...
			$class  = 'icon-preview';
			if ( ! $option && $slug == 'admin-post' ) {
				$class = 'icon-preview active';
			} elseif ( $slug == $option ) {
				$class = 'icon-preview active';
			}
			$list[] = sprintf(
				'<li id="icon-%s" class="%s"><span class="dashicons dashicons-%s"><span class="screen-reader-text">%s</span></span></li>',
				$slug,
				$class,
				$slug,
				__( $name, 'acf' )
	...
	public function menu_icon_script() {
		...
		foreach ( $icons as $slug => $name ) {
			...
			$script .= "if ( show == '{$slug}' ) { $( '#icon-{$slug}' ).fadeIn( 250 ).addClass( 'active' ); } else if ( show != '{$slug}' ) { $( '#icon-{$slug}' ).fadeOut(0).removeClass( 'active' ); }";
		}
		$script .= '}); });</script>';
