...
class acfe_dynamic_block_types extends acf_module {
	...
	public $post_type = 'acf-block-type';
	...
	function register_user_block_types(){
		...
		$settings = apply_filters('acfe/block_type/prepare_register', acf_get_module_settings($this->settings));
	...
	function post_head(){
		...
		if($field_groups){
			...
			add_meta_box('acf-block-type-field-groups', __('Field Groups', 'acf'), array($this, 'metabox_render'), $this->post_type, 'normal', 'default', $field_groups);
	...
	function metabox_render($array, $data){
		...
		(function($){
			...
			acf.newPostbox(<?php echo wp_json_encode(array(
				'id'    => 'acf-block-type-field-groups',
				'key'   => '',
				'style' => 'default',
				'label' => 'left',
				'edit'  => false
