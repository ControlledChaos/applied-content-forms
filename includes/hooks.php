...
function acf_add_filter_variations( $filter = '', $variations = [], $index = 0 ) {
	...
	acf_get_store( 'hook-variations' )->set( $filter, [
		'type'       => 'filter',
	...
	/*
	 * Add generic handler. Use a priority of 10,
	 * and accepted args of 10 (ignored by WP).
	 */
...
function acf_add_action_variations( $action = '', $variations = [], $index = 0 ) {
	...
	// Store replacement data.
	acf_get_store( 'hook-variations' )->set( $action, [
		'type'       => 'action',
		'variations' => $variations,
		'index'      => $index
	] );
...
/**
 * Apply hook variations
 *
 * Apply hook variations during apply_filters() or do_action().
 *
 * @since  1.0.0
 * @return mixed
 */
...
function _acf_apply_deprecated_hook() {
	...
	$items = acf_get_store( 'deprecated-hooks' )->query( [ 'replacement' => $hook ] );
