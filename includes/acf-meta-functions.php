...
function acf_get_meta( $post_id = 0 ) {
	 ...
	 *
	 * Relies on decoded post_id result to identify
	 * option or meta types.
	 * Uses xxx_metadata(type) instead of xxx_type_meta()
	 * to bypass additional logic that could alter the ID.
	 */
	if ( 'option' === $type ) {
 ...
 * Get option meta
 *
 * Returns an array of meta for the given wp_option name
 * prefix in the same format as get_post_meta().
 *
 * @since  1.0.0
 ...
 * @param  boolean $hidden If the meta is hidden
...
function acf_get_metadata( $post_id = 0, $name = '', $hidden = false ) {
	...
	// Stop if no $id (possible during new acf_form).
	if ( ! $id ) {
		return null;
	}
	 ...
	 * Uses xxx_metadata(type) instead of xxx_type_meta() to bypass
	 * additional logic that could alter the ID.
	 */
	if ( 'option' === $type ) {
 ...
 *
 * @since  1.0.0
 * @param  mixed $from_post_id The post id to copy from.
 * @param  mixed $to_post_id The post id to paste to.
 * @return void
 */
 ...
 * Copy postmeta
 *
 * Copies meta from one post to another.
 * Useful for saving and restoring revisions.
 *
 * @deprecated
 *
 * @since  1.0.0
 ...
 * @param  mixed $post_id The post_id where this field's value is saved.
 ...
 * @return mixed Meta ID if the key didn't exist,
