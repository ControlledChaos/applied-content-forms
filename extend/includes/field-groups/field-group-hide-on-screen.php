...
class acfe_field_group_hide_on_screen {
	...
	function __construct() {
		...
		// Post Metaboxes.
		add_action( 'acf/add_meta_boxes', [ $this, 'acf_add_meta_boxes' ], 10, 3 );
		add_action( 'wp_ajax_acf/ajax/check_screen', [ $this, 'ajax_check_screen' ], 9 );
		add_filter( 'acf/get_field_group_style', [ $this, 'get_field_group_style' ], 10, 2 );

		// Hide Block Editor.
		add_action( 'load-post.php', [ $this, 'hide_block_editor' ] );
		add_action( 'load-post-new.php', [ $this, 'hide_block_editor' ] );

	}

	function admin_head() {
		add_filter( 'acf/prepare_field/name=hide_on_screen', [ $this, 'prepare_hide_on_screen' ] );
	}
	...
	function prepare_hide_on_screen( $field ) {
		...
		$choices = [
			'title'           => __( 'Title', 'acf' ),
			'save_draft'      => __( 'Save Draft', 'acf' ),
			'preview'         => __( 'Preview', 'acf' ),
			'post_status'     => __( 'Post Status', 'acf' ),
			'visibility'      => __( 'Post Visibility', 'acf' ),
			'publish_date'    => __( 'Publish Date', 'acf' ),
			'trash'           => __( 'Move to Trash', 'acf' ),
			'publish'         => __( 'Publish/Update', 'acf' ),
			'minor_publish'   => __( 'Minor Publishing Actions', 'acf' ),
			'misc_publish'    => __( 'Misc Publishing Actions', 'acf' ),
			'major_publish'   => __( 'Major Publishing Actions', 'acf' ),
			'publish_metabox' => __( 'Publish Metabox', 'acf' )
		];

		foreach ( $field['choices'] as $key => $value ) {

			if ( 'the_content' === $key ) {
				$choices['block_editor'] = __( 'Block Editor', 'acf' );
			}
			$choices[$key] = $value;
		}
		asort( $choices );
